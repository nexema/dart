  ...
  group("Nexemab encode/decode", () {
    group("encode", () {
      var testcases = TestCaseRunner(<TestCase<void Function(NexemabWriter), List<int>>>[
        ...
        TestCase("int64(maxint64)", (writer) => writer.encodeInt64(Numbers.int64MaxValue), [0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]),
        TestCase("int64(minint64)", (writer) => writer.encodeInt64(Numbers.int64MinValue), [128, 0, 0, 0, 0, 0, 0, 0]),
        ...
        TestCase("encodeUvarint throws out of bounds", (writer) => writer.encodeUvarint(BigInt.from(-124)), [], FormatError("uvarint value must be greater than or equals to 0.")),
      ...
      test("internal buffer should resize successfuly if not big as needed", () {
        ...
        expect(buffer, equals(Uint8List.fromList([0x18, 0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0x21, 0x72, 0xbb, 0x7e, 0xd, 0x47, 0xf0, 0x7a, 0x10, 0xa, 0x7b, 0x7b, 0x36, 0x2b, 0xb])));
      });
    });
    ...
    group("decode primitives", () {
      ...
      var testcases = TestCaseRunner(<TestCase<List<dynamic>, dynamic>>[
        ...
        TestCase("uvarint(267)", [[139, 2], (NexemabReader decoder) => decoder.decodeUvarint()], BigInt.from(267)),
        TestCase("uvarint(59992)", [[216, 212, 3], (NexemabReader decoder) => decoder.decodeUvarint()], BigInt.from(59992)),
        TestCase("uvarint(maxuint32)", [[255, 255, 255, 255, 15], (NexemabReader decoder) => decoder.decodeUvarint()], BigInt.from(Numbers.uint32MaxValue)),
        TestCase("uvarint(maxuint64)", [[255, 255, 255, 255, 255, 255, 255, 255, 255, 1], (NexemabReader decoder) => decoder.decodeUvarint()], Numbers.uint64MaxValue),
        TestCase("uvarint(maxint64)", [[255, 255, 255, 255, 255, 255, 255, 255, 127], (NexemabReader decoder) => decoder.decodeUvarint()], Numbers.int64MaxValueBigInt),
        ...
        TestCase("varint(59992)", [[176, 169, 7], (NexemabReader decoder) => decoder.decodeVarint()], 59992),
        ...
        TestCase("int8(min)", [[128], (NexemabReader decoder) => decoder.decodeInt8()], Numbers.int8MinValue),
        TestCase("int8(max)", [[127], (NexemabReader decoder) => decoder.decodeInt8()], Numbers.int8MaxValue),
        TestCase("int16(min)", [[128, 0], (NexemabReader decoder) => decoder.decodeInt16()], Numbers.int16MinValue),
        TestCase("int16(max)", [[127, 255], (NexemabReader decoder) => decoder.decodeInt16()], Numbers.int16MaxValue),
        TestCase("int32(min)", [[128, 0, 0, 0], (NexemabReader decoder) => decoder.decodeInt32()], Numbers.int32MinValue),
        TestCase("int32(max)", [[127, 255, 255, 255], (NexemabReader decoder) => decoder.decodeInt32()], Numbers.int32MaxValue),
        TestCase("int64(min)", [[128, 0, 0, 0, 0, 0, 0, 0], (NexemabReader decoder) => decoder.decodeInt64()], Numbers.int64MinValue),
        TestCase("int64(max)", [[127, 255, 255, 255, 255, 255, 255, 255], (NexemabReader decoder) => decoder.decodeInt64()], Numbers.int64MaxValue),
        TestCase("int64AsBigInt(max)", [[127, 255, 255, 255, 255, 255, 255, 255], (NexemabReader decoder) => decoder.decodeInt64AsBigInt()], Numbers.int64MaxValueBigInt),
        TestCase("int64AsBigInt(min)", [[128, 0, 0, 0, 0, 0, 0, 0], (NexemabReader decoder) => decoder.decodeInt64AsBigInt()], Numbers.int64MinValueBigInt),
        TestCase("int64AsBigInt(24151241321)", [[0x0, 0x0, 0x0, 0x5, 0x9f, 0x86, 0xb2, 0x69], (NexemabReader decoder) => decoder.decodeInt64AsBigInt()], BigInt.from(24151241321)),
        TestCase("uint8(0)", [[0], (NexemabReader decoder) => decoder.decodeUint8()], 0),
        TestCase("uint8(max)", [[255], (NexemabReader decoder) => decoder.decodeUint8()], Numbers.uint8MaxValue),
        TestCase("uint16(0)", [[0, 0], (NexemabReader decoder) => decoder.decodeInt16()], 0),
        TestCase("uint16(max)", [[255, 255], (NexemabReader decoder) => decoder.decodeUint16()], Numbers.uint16MaxValue),
        TestCase("uint32(0)", [[0, 0, 0, 0], (NexemabReader decoder) => decoder.decodeUint32()], 0),
        TestCase("uint32(max)", [[255, 255, 255, 255], (NexemabReader decoder) => decoder.decodeUint32()], Numbers.uint32MaxValue),
        TestCase("uint64(0)", [[0, 0, 0, 0, 0, 0, 0, 0], (NexemabReader decoder) => decoder.decodeUint64()], BigInt.zero),
        TestCase("uint64(max)", [[255, 255, 255, 255, 255, 255, 255, 255], (NexemabReader decoder) => decoder.decodeUint64()], Numbers.uint64MaxValue),
        TestCase("float32(max)", [[0x7f, 0x7f, 0xff, 0xff], (NexemabReader decoder) => decoder.decodeFloat32()], Numbers.float32MaxValue),
        TestCase("float32(min)", [[0x0, 0x0, 0x0, 0x1], (NexemabReader decoder) => decoder.decodeFloat32()], Numbers.float32MinValue),
        ...
        ["hola", "hello", "tercero", "third", "bye"]),
        TestCase("map(string,varint)", [
          [0xdf, 0x6, 0x2, 0x61, 0x96, 0xad, 0x2f, 0x2, 0x62, 0xc2, 0xe, 0x6, 0x62, 0x79, 0x65, 0xaa, 0xa1, 0xa0, 0xc], 
          (NexemabReader decoder) => generateMap<String, int>(decoder.beginDecodeMap(), () => decoder.decodeString(), () => decoder.decodeVarint())], 
        {"a": 387915, "b": 929, "bye": 12847189}),
        TestCase("list fails if not an array", [
          [0xdf, 0x6, 0x2, 0x61, 0x96, 0xad, 0x2f, 0x2, 0x62, 0xc2, 0xe, 0x6, 0x62, 0x79, 0x65, 0xaa, 0xa1, 0xa0, 0xc], 
          (NexemabReader decoder) => List<String>.generate(decoder.beginDecodeArray(), (index) => decoder.decodeString())], 
        null, FormatError("not an array.")),
        TestCase("map fails if not a map", [
          [0xdc, 0xa, 0x8, 0x68, 0x6f, 0x6c, 0x61, 0xa, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0xe, 0x74, 0x65, 0x72, 0x63, 0x65, 0x72, 0x6f, 0xa, 0x74, 0x68, 0x69, 0x72, 0x64, 0x6, 0x62, 0x79, 0x65], 
          (NexemabReader decoder) => generateMap<String, int>(decoder.beginDecodeMap(), () => decoder.decodeString(), () => decoder.decodeVarint())], 
        null, FormatError("not a map."))
      ]);
